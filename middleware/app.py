...
# Lazy-init Bedrock client (not available in China regions)
bedrock_client = None
...
def get_bedrock_client():
    ...
    if bedrock_client is None:
        try:
            ...
            print(f"Bedrock client not available in this region: {e}")
...
async def convert_bedrock_to_openai(
    ...
    if model_id.startswith("arn:") and ":bedrock:" in model_id:
        ...
        if prompt_id:
            ...
            if _bedrock is None:
                raise HTTPException(status_code=503, detail="Bedrock is not available in this region")
            if prompt_version:
                prompt = _bedrock.get_prompt(
                    promptIdentifier=prompt_id, promptVersion=prompt_version
                )
            else:
                prompt = _bedrock.get_prompt(promptIdentifier=prompt_id)
...
async def proxy_request(request: Request):
    ...
    try:
        ...
        final_prompt_text = None
        if model_id and model_id.startswith("arn:") and ":bedrock:" in model_id:
            prompt_id, prompt_version = parse_prompt_arn(model_id)
            if prompt_id:
                _bedrock = get_bedrock_client()
                if _bedrock is None:
