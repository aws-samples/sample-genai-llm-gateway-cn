...
if [ ! -f "config/config.yaml" ]; then
  ...
  if [ -f "$region_config" ]; then
    ...
    echo "Region-specific config $region_config not found, using base config only for region $aws_region"
    cp "$base_config" "config/config.yaml"
  fi
fi
...
# China region auto-detection (must be after source .env)
if [[ "$aws_region" == cn-* ]]; then
  echo "🇨🇳 China region detected ($aws_region). Forcing USE_CLOUDFRONT=false."
  USE_CLOUDFRONT="false"
  if [ -z "${LITELLM_BASE_IMAGE:-}" ]; then
    echo "⚠️  WARNING: LITELLM_BASE_IMAGE not set. China regions cannot pull from ghcr.io."
    echo "   Run: ./scripts/mirror-images-to-ecr.sh first, then set LITELLM_BASE_IMAGE in .env"
  fi
fi
...
echo "SILICONFLOW_API_KEY: ${SILICONFLOW_API_KEY:-placeholder}"
...
# Convert comma-separated ALLOWED_CIDRS to Terraform list format
if [ -n "${ALLOWED_CIDRS:-}" ]; then
  export TF_VAR_allowed_cidrs=$(echo "$ALLOWED_CIDRS" | python3 -c "import sys,json; print(json.dumps([c.strip() for c in sys.stdin.read().strip().split(',') if c.strip()]))")
else
  export TF_VAR_allowed_cidrs='[]'
fi
...
export TF_VAR_siliconflow_api_key=${SILICONFLOW_API_KEY:-placeholder}
